...
const Page = () => {
  ...
  const handleConfirmTitleAction = async () => {
    ...
    // Check if editingTitle has white space, if yes, show alert message
    if (editingTitle.indexOf(" ") >= 0) {
      alert("頁面標題不可包含空格，請重新輸入");
      return;
    }
    ...
    if (isEditingTitle) {
      return (
        <Box>
          ...
          <Stack sx={sxs.stack} spacing={2} direction="row">
            <Button variant="contained" onClick={handleConfirmTitleAction}>
              {" "}
              確定{" "}
            </Button>
            <Button variant="outlined" onClick={handleCancelTitleAction}>
              {" "}
              取消{" "}
            </Button>
          </Stack>
      ...
      return (
        <Stack sx={sxs.stack} spacing={2} direction="row">
          ...
          {editMode ? (
            <Button variant="outlined" onClick={handleEditTitleAction}>
              {" "}
              更改頁面標題{" "}
            </Button>
          ) : null}
    ...
    cards.forEach((card) => {
      ...
      rows.push(
        <Box maxWidth={700} key={card._id}>
          <Card
            card_data={card_data}
            edit_mode={editMode ? "編輯" : undefined}
            key={card._id}
          />
        </Box>
      );
  ...
  return editMode && !isLoggedIn ? (
    ...
    <Box>
      ...
      <Box sx={sxs.container}>
        <Box sx={sxs.section}>{show_title()}</Box>
        <Box sx={sxs.section}>
          {show_banner_image()}
          {editMode ? (
            <Stack sx={sxs.box} spacing={2} direction="row">
              ...
              <label htmlFor="image">
                <Input
                  ...
                  type="file"
                  name="image"
                  onChange={select_banner_image}
                />
                <Button variant="outlined" component="span">
                  {previewImage || (currentPage && currentPage.banner_image)
                    ? "更改照片"
                    : "選擇照片"}
                </Button>
              </label>
